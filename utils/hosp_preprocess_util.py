...
from pyspark.sql.functions import col, lower, trim, regexp_replace, datediff, expr, to_date
...
spark = SparkSession.builder \
    .appName("preproc_meds_spark") \
    .getOrCreate()

def preproc_meds_spark(module_path: str, adm_cohort_path: str, mapping: str):
    spark = SparkSession.builder.appName("preproc_meds_spark").getOrCreate()
    # Read admissions cohort data
    adm = spark.read.csv(adm_cohort_path, header=True, inferSchema=True).select(['hadm_id', 'admittime'])

    # Read medications data
    med = spark.read.csv(module_path, header=True, inferSchema=True).select(['subject_id', 'hadm_id', 'drug', 'starttime', 'stoptime', 'ndc', 'dose_val_rx'])
    med = med.join(adm, on='hadm_id', how='inner')

    # Calculate start and stop hours from admit
    med = med.withColumn('start_hours_from_admit', expr("unix_timestamp(starttime) - unix_timestamp(admittime)"))
    med = med.withColumn('stop_hours_from_admit', expr("unix_timestamp(stoptime) - unix_timestamp(admittime)"))

    #get strings in same format as time delta
    ...
    med = med.withColumn('drug', lower(col('drug')))
    med = med.withColumn('drug', trim(col('drug')))
    med = med.withColumn('drug', regexp_replace(col('drug'), ' ', '_'))
    med = med.dropna(subset=['drug'])
    med = med.withColumn('drug', lower(col('drug')))
    med = med.withColumn('drug', trim(col('drug')))

    med = ndc_meds(med.toPandas(), mapping)
...
# Initialize Spark session
spark = SparkSession.builder \
    .appName("preproc_labs_spark") \
    .getOrCreate()

def preproc_labs_spark(dataset_path: str, version_path: str, cohort_path: str, time_col: str, anchor_col: str, dtypes: dict, usecols: list):
    """Function for getting hosp observations pertaining to a pickled cohort. Function is structured to save memory when reading and transforming data."""

    spark = SparkSession.builder.appName("preproc_labs_spark").getOrCreate()

    usecols = ['itemid','subject_id','hadm_id','charttime','valuenum','valueuom']

    # Read cohort data
    cohort = spark.read.csv(cohort_path, header=True, inferSchema=True).select(['subject_id', 'hadm_id', 'admittime', 'dischtime'])

    adm = spark.read.csv("./mimiciv/3.0/hosp/admissions.csv.gz", header=True, inferSchema=True).select(['subject_id', 'hadm_id', 'admittime', 'dischtime'])

    # Read module with custom params
    labs = spark.read.csv(dataset_path, header=True, inferSchema=True).select(usecols).dropna(subset=['valuenum'])


    labs = labs.dropna(subset=['valuenum'])
    labs = labs.na.fill(value=0, subset=['valueuom'])

    # Filter labs to only include subjects in the cohort
    labs = labs.filter(col('subject_id').isin([row['subject_id'] for row in cohort.collect()]))
    labs_na = labs.filter(col('hadm_id').isNull())
    labs = labs.filter(col('hadm_id').isNotNull())
    
    #had to switch to pandas
    labs_na = impute_hadm_ids_spark(labs_na.select('subject_id','hadm_id','itemid','charttime','valuenum','valueuom'), adm)

    #labs_na = spark.createDataFrame(labs_na)

    #labs_na = labs_na.drop('hadm_id')

    #labs_na = labs_na.withColumnRenamed('hadm_id_new', 'hadm_id')
    labs_na = labs_na.select('subject_id','hadm_id','itemid','charttime','valuenum','valueuom')
    labs = labs.union(labs_na)
    labs = labs.join(cohort.select('hadm_id', 'admittime', 'dischtime'), how='inner', on='hadm_id')

    labs = labs.withColumn('charttime', col('charttime').cast("timestamp"))

    labs = labs.withColumn('lab_time_from_admit', expr("unix_timestamp(charttime) - unix_timestamp(admittime)"))

    labs = labs.withColumn("lab_time_from_admit", expr("""
    concat(
        floor(lab_time_from_admit / 86400), ' days ',
        lpad(floor((lab_time_from_admit % 86400) / 3600), 2, '0'), ':',
        lpad(floor((lab_time_from_admit % 3600) / 60), 2, '0'), ':',
        lpad(lab_time_from_admit % 60, 2, '0')
    )
    """))
    ...
    print("# Itemid: ", labs.select('itemid').distinct().count())
    print("# Admissions: ", labs.select('hadm_id').distinct().count())
    print("Total number of rows: ", labs.count())

    labs = labs.orderBy('subject_id')
    labs = labs.toPandas()
    spark.stop()
...
def preproc_proc_spark(dataset_path: str, cohort_path: str, time_col: str, anchor_col: str, dtypes: dict, usecols: list):
    """Function for getting hosp observations pertaining to a pickled cohort. Function is structured to save memory when reading and transforming data."""

    spark = SparkSession.builder.appName("preproc_proc_spark").getOrCreate()
    ...
    def merge_module_cohort(spark):
        ...
        if usecols is not None:
          # Read module with custom params
          module = spark.read.csv(dataset_path, header=True, inferSchema=True).select(usecols).dropDuplicates()
        else:
          module = spark.read.csv(dataset_path, header=True, inferSchema=True).dropDuplicates()

        # Only consider values in our cohort
        cohort = spark.read.csv(cohort_path, header=True, inferSchema=True).select(['hadm_id', 'admittime', 'dischtime'])

        # Merge module and cohort
        return module.join(cohort, on='hadm_id', how='inner')

    df_cohort = merge_module_cohort(spark)

    df_cohort = df_cohort.withColumn('proc_time_from_admit', expr("unix_timestamp(chartdate) - unix_timestamp(admittime)"))

    df_cohort = df_cohort.withColumn("proc_time_from_admit", expr("""
    concat(
        floor(proc_time_from_admit / 86400), ' days ',
        lpad(floor((proc_time_from_admit % 86400) / 3600), 2, '0'), ':',
        lpad(floor((proc_time_from_admit % 3600) / 60), 2, '0'), ':',
        lpad(proc_time_from_admit % 60, 2, '0')
    )
    """))
    ...
    # Print unique counts and value_counts
    print("# Unique ICD9 Procedures:  ", df_cohort.filter(col('icd_version') == 9).select('icd_code').dropna().distinct().count())
    print("# Unique ICD10 Procedures: ", df_cohort.filter(col('icd_version') == 10).select('icd_code').dropna().distinct().count())

    print("\nValue counts of each ICD version:\n", df_cohort.groupBy('icd_version').count().orderBy('count'))
    print("# Admissions:  ", df_cohort.select('hadm_id').distinct().count())
    print("Total number of rows: ", df_cohort.count())

    df_cohort = df_cohort.orderBy('subject_id').toPandas()
