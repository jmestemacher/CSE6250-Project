...
class BEHRT_models():
    ...
    def tokenize_dataset(self,labs_input, cond_input, demo_input, labels, vocab, demo_vocab, ins_vocab, gender_vocab):
        ...
        for patient, group in tqdm.tqdm(labs_input.groupby(self.id)):
            ...
            if len(tokenized_src[idx]) >= 512:
                ...
                #Fix bug in base code where insurance is nan (set to 'Other' in that case)
                try:
    ...
    def tokenize(self):
        ...
        #fixed a minor bug in the base code here by converting insVocab_reversed[i] to a string
        for i in range(len(insVocab_reversed)):
            print(str(len(tokenized_ins[tokenized_ins[0] == i])) + " " + str(insVocab_reversed[i]) + " samples.")
