...
#         print(labels.shape)
        ...
        labels = labels.squeeze()  # Remove extra dimensions if needed
        prob = prob.squeeze()
        ...
        #################           BCE            #######################
        if len(pos_prob):
            pos_prob = pos_prob.squeeze()  # or neg_prob = neg_prob.view(-1)
            pos_label = pos_label.squeeze()  # optional, if needed
            pos_prob=pos_prob.to(self.device)
            pos_label=pos_label.to(self.device)
            pos_loss = self.classify_loss(pos_prob, pos_label) 
       
        if len(neg_prob):
            neg_prob = neg_prob.squeeze()  # or neg_prob = neg_prob.view(-1)
            neg_label = neg_label.squeeze()  # optional, if needed
