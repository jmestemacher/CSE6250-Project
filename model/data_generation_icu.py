...
from pyspark.sql.functions import col, lit, when, collect_list, array, struct, monotonically_increasing_id, row_number
...
#         else:
    ...
    def create_Dict_spark(self, meds, proc, out, chart, los):
        """
        Create dictionaries and save data for each stay_id using PySpark.

        Parameters:
        meds: PySpark DataFrame containing medication data.
        proc: PySpark DataFrame containing procedure data.
        out: PySpark DataFrame containing output event data.
        chart: PySpark DataFrame containing chart event data.
        los: Length of stay (in hours).
        """
        # spark = SparkSession.builder.appName("CreateDict").config("spark.sql.shuffle.partitions", "8").config("spark.default.parallelism", "8").getOrCreate()
        spark = SparkSession.builder.appName("CreateDict").getOrCreate()
        self.data_spark = spark.createDataFrame(self.data)

        # Initialize dictionaries and labels DataFrame
        dataDic = {}
        ...
        if self.feat_med and not meds.empty:
          meds = spark.createDataFrame(meds)
        # proc = spark.createDataFrame(proc)
        # out = spark.createDataFrame(out)
        # chart = spark.createDataFrame(chart)
        ...
        for hid in tqdm(self.hids):
            ...
            demo_csv = self.data_spark.filter(col("stay_id") == hid).select("Age", "gender", "ethnicity", "insurance").toPandas()
            if not os.path.exists("./data/csv/"+str(hid)):
                os.makedirs("./data/csv/"+str(hid))
            demo_csv.to_csv('./data/csv/'+str(hid)+'/demo.csv',index=False)
            
            # Initialize dynamic CSV
            dyn_csv = None

            # Process medications
            if self.feat_med and not meds.isEmpty():
                feat = meds.select("itemid").distinct().rdd.flatMap(lambda x: x).collect()
                df2 = meds.filter(col("stay_id") == hid)
                if df2.isEmpty():
                    ...
                    amount = spark.createDataFrame(amount)
                else:
                    ...
                    add_indices = pd.Index(range(los)).difference(df2.index)
                    add_df = pd.DataFrame(index=add_indices, columns=df2.columns).fillna(np.nan)
                    add_df = add_df.reset_index().rename(columns={'index': 'start_time'})
                    rate = rate.reset_index().rename(columns={'index': 'start_time'})
                    df2 = df2.reset_index().rename(columns={'index': 'start_time'})
                    amount = amount.reset_index().rename(columns={'index': 'start_time'})
                    add_df = spark.createDataFrame(add_df)
                    df2 = spark.createDataFrame(df2)
                    amount = spark.createDataFrame(df2)
                    rate = spark.createDataFrame(rate)
                    df2 = df2.union(add_df)
                    #unpersist to avoid out of memory errors
                    add_df.unpersist()

                    df2 = df2.orderBy("start_time")

                    # Forward fill (ffill equivalent) using window function
                    windowSpec = Window.orderBy("start_time").rowsBetween(-float("inf"), 0)

                    # Apply forward fill using last value in the window
                    df2 = df2.select(
                        *[F.last(c, True).over(windowSpec).alias(c) for c in df2.columns]
                    )
                    # Fill missing values
                    df2 = df2.fillna(0)
                    rate = rate.union(add_df)
                    rate = rate.orderBy("start_time")
                    ...
                    # Apply forward fill using last value in the window
                    rate = rate.select(
                        *[F.last(c, True).over(windowSpec).alias(c) for c in rate.columns]
                    )
                    rate = rate.fillna(-1)
                    amount = amount.union(add_df)
                    amount = amount.orderBy("start_time")

                    # Forward fill (ffill equivalent) using window function
                    windowSpec = Window.orderBy("start_time").rowsBetween(-float("inf"), 0)
                    # Apply forward fill using last value in the window
                    amount = amount.select(
                        *[F.last(c, True).over(windowSpec).alias(c) for c in amount.columns]
                    )
                    amount = amount.fillna(-1)

                    df2 = df2.select(
                        *[F.col(col) - F.col("start_time").cast("double").alias(col) if col != "start_time" else F.col(col) for col in df2.columns]
                    )
                    #process signals
                    df2 = df2.select(
                        *[F.when(F.col(c) > 0, 1)
                    ...
                    rate = rate.select(
                        *[F.col(c) * F.col(c).alias(c) for c in df2.columns]
                    )
                    amount = amount.select(
                        *[F.col(c) * F.col(c).alias(c) for c in df2.columns]
                    )
                    ...
                    # # Process signals
                    # df2 = df2.withColumn("signal", when(col("stop_time") > 0, 1).otherwise(0))
                #    rate = rate.withColumn("rate", col("signal") * col("rate"))
                #   amount = amount.withColumn("amount", col("signal") * col("amount"))

                    # dataDic[hid]["Med"]["signal"] = df2.toPandas().to_dict(orient="list")
                    # dataDic[hid]["Med"]["rate"] = rate.toPandas().to_dict(orient="list")
                    # dataDic[hid]["Med"]["amount"] = amount.toPandas().to_dict(orient="list")

                    df2_dict = {col: [row[col] for row in df2.collect()] for col in df2.columns}
                    rate_dict = {col: [row[col] for row in rate.collect()] for col in rate.columns}
                    amount_dict = {col: [row[col] for row in amount.collect()] for col in amount.columns}

                    # Now, assign these dictionaries to the dataDic
                    dataDic[hid]['Med']['signal'] = df2_dict
                    dataDic[hid]['Med']['rate'] = rate_dict
                    dataDic[hid]['Med']['amount'] = amount_dict

                    df2.unpersist()
                    rate.unpersist()

                    feat_df=pd.DataFrame(columns=list(set(feat)-set(amount.columns)))
                    amount = amount.toPandas()
    ...
    #                 print(amount.columns)
                ...
                if dyn_csv is None:
                    dyn_csv = amount.select('*')
                    amount.unpersist()
                else:
                  dyn_csv = dyn_csv.withColumn('row_index', row_number().over(Window.orderBy(monotonically_increasing_id())))
                  amount = amount.withColumn('row_index', row_number().over(Window.orderBy(monotonically_increasing_id())))
                  dyn_csv = dyn_csv.join(amount, on=["row_index"]).drop("row_index")
                  amount.unpersist()
            ...
            if self.feat_proc:
                ...
                if df2.empty:
                    ...
                    df2 = df2.select(
                        *[
                            F.when(F.col(col) > 0, 1).otherwise(F.col(col)).alias(col)
                            for col in df2.columns
                        ]
                    )

                    df2_dict = {col: [row[col] for row in df2.collect()] for col in df2.columns}
                    dataDic[hid]['Proc'] = df2_dict
                ...
                if dyn_csv is None:
                  ...
                  dyn_csv = dyn_csv.join(df2, on="row_index").drop("row_index")
                  df2.unpersist()
            ...
            if self.feat_chart:
                ...
                if df2.empty:
                    ...
                    df2 = df2.reset_index().rename(columns={'index': 'start_time'})
                    add_df = add_df.reset_index().rename(columns={'index': 'start_time'})
                    val = val.reset_index().rename(columns={'index': 'start_time'})
                    add_df = spark.createDataFrame(add_df)
                    df2 = spark.createDataFrame(df2)
                    df2 = df2.union(add_df)
                    df2 = df2.fillna(0)
                    val = val.union(add_df)
                    add_df.unpersist()
                    val = val.orderBy('start_time')
                    if self.impute=='Mean':
                        # Forward fill
                        window_spec = Window.partitionBy().orderBy("start_time").rowsBetween(Window.unboundedPreceding, 0)
                        pivot_df = pivot_df.select(*[F.last(c, True).over(window_spec).alias(c) if c != 'start_time' else c for c in pivot_df.columns])

                        # Backward fill
                        window_spec_reverse = Window.partitionBy().orderBy("start_time").rowsBetween(0, Window.unboundedFollowing)
                        pivot_df = pivot_df.select(*[F.last(c, True).over(window_spec_reverse).alias(c) if c != 'start_time' else c for c in pivot_df.columns])

                        # impute using mean
                        mean_values = pivot_df.select(*[F.mean(c).alias(c) for c in pivot_df.columns if c != 'start_time']).collect()[0].asDict()
                        val = pivot_df.fillna(mean_values)
                    elif self.impute=='Median':
                        # Forward fill
                        window_spec = Window.partitionBy().orderBy("start_time").rowsBetween(Window.unboundedPreceding, 0)
                        pivot_df = pivot_df.select(*[F.last(c, True).over(window_spec).alias(c) if c != 'start_time' else c for c in pivot_df.columns])
                        ...
                        # impute using mean
                        median_values = pivot_df.select(*[F.median(c).alias(c) for c in pivot_df.columns if c != 'start_time']).collect()[0].asDict()
                        val = pivot_df.fillna(median_values)
                    val = val.fillna(0)

                    df2 = df2.select(
                        *[
                            F.when(col > 0, 1).when(col < 0, 0).otherwise(col).alias(col_name) 
                            for col_name, col in df2.columns
                        ]
                    )

                    df2_dict = {col: [row[col] for row in df2.collect()] for col in df2.columns}
                    val_dict = {col: [row[col] for row in val.collect()] for col in val.columns}

                    # Now, assign these dictionaries to the dataDic
                    dataDic[hid]['Chart']['signal'] = df2_dict
                    dataDic[hid]['Chart']['val'] = val_dict
                    df2.unpersist()
                ...
                if dyn_csv is None:
                  ...
                  dyn_csv = dyn_csv.withColumn('row_index', row_number().over(Window.orderBy(monotonically_increasing_id())))
                  val = val.withColumn('row_index', row_number().over(Window.orderBy(monotonically_increasing_id())))
                  dyn_csv = dyn_csv.join(val, on=["row_index"]).drop("row_index")
                  val.unpersist()

            # Save dynamic data to CSV
            dyn_csv.toPandas().to_csv('./data/csv/'+str(hid)+'/dynamic.csv',index=False)




            ##########COND#########
            #not converting to Spark as the overhead of it will likely be too much
            if(self.feat_cond):
