...
def feature_icu(cohort_output, version_path, diag_flag=True,out_flag=True,chart_flag=True,proc_flag=True,med_flag=True):
    if diag_flag:
        ...
        diag = preproc_icd_module_spark("./"+version_path+"/hosp/diagnoses_icd.csv.gz", './data/cohort/'+cohort_output+'.csv.gz', './utils/mappings/ICD9_to_ICD10_mapping.txt', map_code_colname='diagnosis_code')
    ...
    if out_flag:  
        ...
        out = preproc_out_spark("./"+version_path+"/icu/outputevents.csv.gz", './data/cohort/'+cohort_output+'.csv.gz', 'charttime', dtypes=None, usecols=None)
    ...
    if chart_flag:
        ...
        chart=preproc_chart_spark("./"+version_path+"/icu/chartevents.csv.gz", './data/cohort/'+cohort_output+'.csv.gz', 'charttime', dtypes=None, usecols=['stay_id','charttime','itemid','valuenum','valueuom'])
    ...
    if proc_flag:
        ...
        proc = preproc_proc_spark("./"+version_path+"/icu/procedureevents.csv.gz", './data/cohort/'+cohort_output+'.csv.gz', 'starttime', dtypes=None, usecols=['stay_id','starttime','itemid'])
    ...
    if med_flag:
        ...
        med = prepoc_meds_spark("./"+version_path+"/icu/inputevents.csv.gz", './data/cohort/'+cohort_output+'.csv.gz')
...
def generate_summary_icu_spark(diag_flag, proc_flag, med_flag, out_flag, chart_flag):
    print("[GENERATING FEATURE SUMMARY]")
    spark = SparkSession.builder.appName("generate_summary_spark").getOrCreate()

    if diag_flag:
        diag = pd.read_csv("./data/features/preproc_diag_icu.csv.gz", compression='gzip',header=0)
        diag_spark = spark.createDataFrame(diag)
        freq = diag_spark.groupBy("stay_id", "new_icd_code").count().withColumnRenamed("count", "mean_frequency")
        freq = freq.groupBy('new_icd_code').agg(avg('mean_frequency')).alias('mean_frequency')
        total = diag_spark.groupBy('new_icd_code').count().withColumnRenamed('count', 'total_count')
        summary = freq.join(total, how='right', on='new_icd_code')
        summary = summary.fillna(0)
        summary.toPandas().to_csv('./data/summary/diag_summary.csv',index=False)
        summary.toPandas()['new_icd_code'].to_csv('./data/summary/diag_features.csv',index=False)
    
    if med_flag:
        med = pd.read_csv("./data/features/preproc_med_icu.csv.gz", compression='gzip',header=0)
        med_spark = spark.createDataFrame(med)
        freq = med_spark.groupBy("stay_id", "itemid").count().withColumnRenamed("count", "mean_frequency")
        freq = freq.groupBy('itemid').agg(avg('mean_frequency')).alias('mean_frequency')
        missing = med_spark.filter(col('amount')==0).groupBy('itemid').count().withColumnRenamed("count", "missing_count")
        total = med_spark.groupBy('itemid').count().withColumnRenamed("count", "total_count")
        summary = missing.join(total, how='right', on='itemid')
        summary = freq.join(summary, how='right', on='itemid')
        #summary['missing%']=100*(summary['missing_count']/summary['total_count']) # comment originally was in Pandas, 
        # left here for reference sake. -Josh
        summary = summary.fillna(0)
        summary.toPandas().to_csv('./data/summary/med_summary.csv',index=False)
        summary.toPandas()['itemid'].to_csv('./data/summary/med_features.csv',index=False)
    
    if proc_flag:
        proc = pd.read_csv("./data/features/preproc_proc_icu.csv.gz", compression='gzip',header=0)
        proc_spark = spark.createDataFrame(proc)
        freq = proc_spark.groupBy("stay_id", "itemid").count().withColumnRenamed("count", "mean_frequency")
        freq = freq.groupBy('itemid').agg(avg('mean_frequency')).alias('mean_frequency')
        total = proc_spark.groupBy('itemid').count().withColumnRenamed('count', 'total_count')
        summary = freq.join(total, how='right', on='itemid')
        summary = summary.fillna(0)
        summary.toPandas().to_csv('./data/summary/proc_summary.csv',index=False)
        summary.toPandas()['itemid'].to_csv('./data/summary/proc_features.csv',index=False)
    
    if out_flag:
        out = pd.read_csv("./data/features/preproc_out_icu.csv.gz", compression='gzip',header=0)
        out_spark = spark.createDataFrame(out)
        freq = out_spark.groupBy("stay_id", "itemid").count().withColumnRenamed("count", "mean_frequency")
        freq = freq.groupBy('itemid').agg(avg('mean_frequency')).alias('mean_frequency')
        total = out_spark.groupBy('itemid').count().withColumnRenamed('count', 'total_count')
        summary = freq.join(total, how='right', on='itemid')
        summary = summary.fillna(0)
        summary.toPandas().to_csv('./data/summary/out_summary.csv',index=False)
        summary.toPandas()['itemid'].to_csv('./data/summary/out_features.csv',index=False)
    
    if chart_flag:
        chart=pd.read_csv("./data/features/preproc_chart_icu.csv.gz", compression='gzip',header=0)
        chart_spark = spark.createDataFrame(chart)
        freq = chart_spark.groupBy("stay_id", "itemid").count().withColumnRenamed("count", "mean_frequency")
        freq = freq.groupBy('itemid').agg(avg('mean_frequency')).alias('mean_frequency')
        missing = chart_spark.filter(col('valuenum')==0).groupBy('itemid').count().withColumnRenamed("count", "missing_count")
        total = chart_spark.groupBy('itemid').count().withColumnRenamed('count', 'total_count')
        summary = missing.join(total, how='right', on='itemid')
        summary = freq.join(summary, how='right', on='itemid')
        #summary['missing_perc']=100*(summary['missing_count']/summary['total_count'])
        #summary=summary.fillna(0)
...
#         final.groupby('itemid')['missing%'].mean().reset_index()
        summary = summary.fillna(0)
        summary.toPandas().to_csv('./data/summary/chart_summary.csv',index=False)
        summary.toPandas()['itemid'].to_csv('./data/summary/chart_features.csv',index=False)     
...
def features_selection_icu_spark(cohort_output, diag_flag,proc_flag,med_flag,out_flag,chart_flag,group_diag,group_med,group_proc,group_out,group_chart):
    spark = SparkSession.builder.appName("feature_selection_spark").getOrCreate()
    ...
    if diag_flag:
        if group_diag:
            ...
            diag_spark = diag_spark.filter(col('new_icd_code').isin([row['new_icd_code'] for row in features_spark.collect()]))
            print("Total number of rows", diag_spark.count())
            diag_spark.toPandas().to_csv("./data/features/preproc_diag_icu.csv.gz", compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED DIAGNOSIS DATA]")
    ...
    if med_flag:
        if group_med:
            ...
            med = med.filter(col('itemid').isin([row['itemid'] for row in features.collect()]))
            print("Total number of rows", med.count())
            med.toPandas().to_csv('./data/features/preproc_med_icu.csv.gz', compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED MEDICATIONS DATA]")
    ...
    if proc_flag:
        if group_proc:
            ...
            proc = proc.filter(col('itemid').isin([row['itemid'] for row in features.collect()]))
            print("Total number of rows", proc.count())
            proc.toPandas().to_csv("./data/features/preproc_proc_icu.csv.gz", compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED PROCEDURES DATA]")
    ...
    if out_flag:
        if group_out:
            ...
            features = spark.createDataFrame(features)
            out = out.filter(col('itemid').isin([row['itemid'] for row in features.collect()]))
            print("Total number of rows", out.count())
            out.toPandas().to_csv("./data/features/preproc_out_icu.csv.gz", compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED OUTPUT EVENTS DATA]")
    ...
    if chart_flag:
        if group_chart:
            ...
            features = spark.createDataFrame(features)
            chart = chart.filter(col('itemid').isin([row['itemid'] for row in features.collect()]))
            print("Total number of rows", chart.count())
            chart.toPandas().to_csv("./data/features/preproc_chart_icu.csv.gz", compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED CHART EVENTS DATA]")
